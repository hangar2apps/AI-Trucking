...
import { DEMO_LOAD_REFERENCE, getLoad, isUsingMockData } from "@/lib/data-provider";
...
export default function LoadDetailPage({ params }: PageProps) {
  const [load, setLoad] = useState<Awaited<ReturnType<typeof getLoad>> | null>(null);
  ...
  if (error && !load) {
    return (
      <div className="p-6">
        ...
        <Link href="/app" className="mt-4 inline-block text-[#0B5FFF]">
          ← Back to dashboard
        </Link>
      </div>
    );
    ...
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-auto p-6">
        ...
        <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
          ...
          <Link
            href={`/app/assistant?load=${load.id}`}
            className="rounded-full bg-[#0B5FFF] px-6 py-2 text-sm font-medium text-white hover:bg-[#0847CC]"
          >
